...
# Install the project into `/docs`
WORKDIR /docs

# Default debug to False, it can be change to true in .env files
ENV DEBUG=False

# Set the virtual environment path to be outside the /docs directory
...
RUN --mount=type=cache,target=/root/.cache/uv \
    --mount=type=bind,source=docs/uv.lock,target=uv.lock \
    --mount=type=bind,source=docs/pyproject.toml,target=pyproject.toml \
